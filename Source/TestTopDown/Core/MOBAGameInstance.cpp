...
{
	serverIndex = idx;
	JoinSessionWithIdx(idx);
}
void UMOBAGameInstance::JoinSessionWithIdx(uint32 SessionIdx)
{
	if (sessionInterface.IsValid() == false) return;
	if (sessionSearch.IsValid() == false) return;
	...
	sessionInterface->JoinSession(0, SESSION_NAME, sessionSearch->SearchResults[SessionIdx]);
}
...
TSubclassOf<ABaseBuilding> UMOBAGameInstance::GetBuildPreviewBP(EBuildable Type)
{
	if (BuildableDataAsset == nullptr)
		return nullptr;
	if (BuildableDataAsset->BuildableMap.Contains(Type) == false)
	...
	return BuildableDataAsset->BuildableMap[Type].BuildPreviewBP;
}
